  ...
  const patchNote = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        await response.json().then((data) =>{
        patchNoteState(data._id, note.title, note.content); // Assuming postNoteState also updates the state for existing notes
        window.location.reload();
        close() }); // Close the dialog after successful update
      }
    } catch (error) {
