...
function App() {
  ...
  const deleteNote = async (entry) => {
    try {
      const response = await fetch(`http://localhost:4000/deleteNote/${entry._id}`, { method: 'DELETE' });
      if (response.ok) {
  ...
  const deleteAllNotes = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.error("Failed to delete all notes:", response.status);
      }
    } catch (error) {
  ...
  const patchNoteState = (id, title, content) => {
    setNotes((prevNotes) =>
    ...
    <div className="App">
      <header className="App-header">
        <div style={dialogOpen ? AppStyle.dimBackground : {}}>
          <h1 style={AppStyle.title}>QuickNotes</h1>
